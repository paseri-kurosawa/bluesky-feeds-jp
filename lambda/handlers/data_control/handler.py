...
def backfill_previous_day(bucket, getfeed_stats_raw_dense, getfeed_stats_stablehashtag):
    ...
    If either was restored, update both processing_trends at the end.
    """
    try:
        now = get_jst_now()
        ...
        # === If either was restored, update both processing_trends ===
        if restored:
            ...
            print(f"[BACKFILL] No restore needed, skipping trends update")
...
def save_hashtag_batch(bucket, hashtags, selected_hot_tags=None, selection_method=None):
    ...
    try:
        ...
        # Also save to components/latest_batch.json for dashboard access
        s3_client.put_object(
            Bucket=bucket,
            Key="components/latest_batch.json",
            Body=body,
            ContentType="application/json; charset=utf-8"
        )
        print(f"[HASHTAG] Saved latest_batch to components/latest_batch.json")

        # Save recent batches (current + previous N-1) individually for dashboard
        config = get_config()
        ...
        try:
            ...
            if "Contents" in response:
                ...
                for bf in batch_files[:hot_batch_lookback - 1]:
                    try:
                        ...
                        batch_ts = bf["Key"].replace("hashtags/batch/", "").replace(".json", "")
                        recent_batches_list.append({"timestamp": batch_ts, "hashtags": bd.get("hashtags", bd)})
                    except Exception:
                        pass

            recent_body = json.dumps({
                "batches": recent_batches_list,
                "selected_hot_tags": selected_hot_tags or [],
                "selection_method": selection_method
            ...
            s3_client.put_object(
                Bucket=bucket,
                Key="components/recent_batches.json",
                Body=recent_body,
                ContentType="application/json; charset=utf-8"
            )
            print(f"[HASHTAG] Saved {len(recent_batches_list)} individual batches to components/recent_batches.json")
        except Exception as e:
            print(f"[HASHTAG] Error saving recent batches: {str(e)}")
    except Exception as e:
